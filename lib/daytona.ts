...
const { comments, creatorContext, nebiusApiKey, nebiusBaseUrl, judgeModel } = input;
...
export async function evaluateInSandbox(
  ...
  try {
    ...
    const input = {
      ...
      nebiusBaseUrl: process.env.NEBIUS_BASE_URL ?? 'https://api.studio.nebius.ai/v1/',
      judgeModel: process.env.JUDGE_MODEL ?? 'Qwen/Qwen3-32B',
    }
