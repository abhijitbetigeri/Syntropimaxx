...
export async function scrapeYouTube(url: string): Promise<NormalizedContent> {
  // Run video metadata + comments scraper in parallel
  const [videoItems, commentItems] = await Promise.all([
    runActorSync('h7sDV53CddomktSi5', {
      startUrls: [{ url }],
      maxResults: 1,
    ...
    runActorSync('p7UMdpQnjKmmpR21D', {
      startUrls: [{ url }],
      maxComments: 20,
  ...
  const rawComments = commentItems
